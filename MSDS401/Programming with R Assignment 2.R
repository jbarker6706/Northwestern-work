...
# *grid.arrange()*. The left barplot will show Poisson probabilties for outcomes 
...
# the probabilty of obtaining "0" is 0.215.  
...
# probabilties versus the corresponding outcomes.  Detemine the value 
...
#(i) (3 points) Identify any observations in "faithful" for which the waiting time 
# exceeds 70 minutes and the eruptions are less than 3.0 minutes.  List and show any
# such observations in a distinct color on a scatterplot of all eruption (vertical axis)
# and waiting times (horizontal axis). Include a horizontal line at eruption = 3.0,
# and a vertical line at waiting time = 70.  Add a title and appropriate text. 

#```{r test2ai}
pabt<-subset(x, subset = waiting>=70 & eruptions <= 3)
pabt
summary(pabt)
str(pabt)

color = rep(NA, length=length(x))
color[which(x$eruptions<3 & x$waiting > 70)] = "red"
color[which(!(x$eruptions<3 & x$waiting > 70))] = "blue"
color
...
#The plot suggests the longer the wit for Faithful to erupt the longer the eruption lasts. 
#In particular there is only one eruption less than three minutes where the wait time for 
#the eruption is more than 70.
#-----
...
#(3)(d) (6 points) Calculate and present a one-sided, 95% confidence interval for the
# average weight gain from day 20 to day 21. Write the code for the paired t-test and
# for determination of the confidence interval endpoints. **Do not use *t.test()**, 
# although you may check your answers using this function. Present the resulting 
# test statistic value, critical value, p-value and confidence interval.
...
##### (4) Statistical inference depends on using a sampling distribution for a 
# statistic in order to make confidence statements about unknown population parameters. 
# The Central Limit Theorem is used to justify use of the normal distribution as a 
# sampling distribution for statistical inference. Using Nile River flow data from 
# 1871 to 1970, this problem demonstrates sampling distribution convergence to normality. 
# Use the code below to prepare the data.  Refer to this example when completing (4)(c) below.
...
# row for "sample1", the second row for "sample2" and the columns labled for each statistic.
